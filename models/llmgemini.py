...
def query_gemini_for_info(conversation_data):
    ...
    print("Gemini LLM full response:", response)
    
    try:
        # Look for JSON-like block wrapped in triple backticks
        start_idx = response.text.find("```json")
        end_idx = response.text.find("```", start_idx + 7)
        
        if start_idx != -1 and end_idx != -1:
            json_str = response.text[start_idx + 7:end_idx].strip()
            #parse JSON string
        ...
        print("Error: Unable to parse the extracted JSON.")
        return {}
...
def clean_text(text):
    """Lowercase and remove punctuation from text: clean data """
